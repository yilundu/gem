...
p.add_argument('--logging_root', type=str, default=os.path.join('/home/gridsan/yilundu/my_files/function-space-gan', 'latent_gan'), help='root for logging')
...
def multigpu_train(gpu, opt, shared_dict, shared_mask):
    ...
    def create_dataloader_callback(sidelength, batch_size):
        ...
        sparsity_range = min(4096, sidelength**2)
    ...
    model = high_level_models.SirenImplicitGAN(num_items=29000, test_num_items=1000, hidden_layers=3,
    ...
    if 'test_latents.weight' not in state_dict:
        state_dict['test_latents.weight'] = model_state_dict['test_latents.weight']

    # state_dict['scale_factor'] = model_state_dict['scale_factor']
    ...
    optimizers = [torch.optim.Adam(lr=opt.lr, params=model.test_latents.parameters())]
    ...
    training.multiscale_training(model=model, ema_model=ema_model, dataloader_callback=create_dataloader_callback, dataloader_iters=(10000000,),
                                 ...
                                 clip_grad=False, val_loss_fn=val_loss_fn, overwrite=True, gpus=opt.gpus, rank=gpu, optimizers=optimizers)
