...
def multigpu_train(gpu, opt, shared_dict, shared_mask):
    ...
    sidelength = 64
    train_img_dataset = dataio.CelebAHQ(sidelength, cache=shared_dict, cache_mask=shared_mask, split='train')
    train_generalization_dataset = dataio.GeneralizationWrapper(train_img_dataset, context_sparsity='full',
                                                                query_sparsity='full',
    ...
    model = high_level_models.SirenImplicitGAN(num_items=num_samples, hidden_layers=3,
                                               amortized=False, latent_dim=1024, noise=True, pos_encode=True, tanh_output=True, type=opt.type).cuda()
    ...
    ix = 0
    # close_idxs = []
    close_idxs = np.load("close.npy")

    for counter, ix in enumerate(range(100, 150)):
        with torch.no_grad():
            ...
            latents_weight = model.latents.weight[None, :, :].view(29000, 1024)
            select_weight = model.latents.weight[ix:ix+1].view(1, 1024)

            dist_val = torch.norm(latents_weight - select_weight, dim=-1)

            _, idx = torch.topk(dist_val, 10, dim=0, largest=False)
            idx = idx.squeeze()

            ims = []
            idx_other = idx[1]
            idx, idx_other = close_idxs[counter]

            latent = model.latents.weight[ix]
            latent_other = model.latents.weight[idx_other]

            _, full_label = train_generalization_dataset[idx_other]
            rgb = full_label['rgb'].view(64, 64, 3).detach().numpy()

            _, full_label = train_generalization_dataset[ix]
            rgb_before = full_label['rgb'].view(64, 64, 3).detach().numpy()

            interval = torch.linspace(0, 1.0, 10).cuda()
            diff = latent_other - latent

            latent_full = latent[None, :].repeat(10, 1)
            latent_full[:, :] = latent[None, :] + interval[:, None] * diff[None, :]

            model_output = model.forward_with_latent(latent_full, full_ctx)
            model_output = model_output.permute(0, 2, 1).view(10, 3, 64, 64)
            output = make_grid(model_output, 10, padding=0)
            im = output.permute(1, 2, 0).cpu().detach().numpy()
            im = np.concatenate([rgb_before, im, rgb], axis=1)
            ims.append(im)

            im_full = np.concatenate(ims, axis=0)
            imwrite("interpolation_no_lle_{}.png".format(ix), im_full)

            # close_idxs.append((ix, idx_other.item()))

    close_idxs = np.array(close_idxs)
    np.save("close.npy", close_idxs)
